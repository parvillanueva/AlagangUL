<div class="box">
    ...
    <div class="box-body">
        <?php
            ...
            $top_details = $this->load->details("tbl_division", $id);
            $inputs = [
                'division_name',
                'status'
            ];
...
$(document).on('click', '#btn_update', function(){
        ...
        if(validate.all()){;    
                if((!is_exist('tbl_division', 'name', $('#name').val(),table_id) != 0)){
                    var modal_obj = '<?= $this->standard->confirm("confirm_update"); ?>'; 
                    modal.standard(modal_obj, function(result){
                        if(result){
                            modal.loading(true);

                            AJAX.update.table("tbl_division");
                            ...
                            AJAX.update.exec(function(result){
                                ...
                                modal.alert("<?= $this->standard->dialog("update_success"); ?>", function(){
                                    location.href = '<?=base_url("content_management/site_division") ?>';
                                });
                            })
                        }
                    });          
                    ...
                    var error_message1 = "<span class='validate_error_message' style='color: red;'>Division Name Exists<br></span>"
