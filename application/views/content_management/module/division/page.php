...
<script type="text/javascript">
    ...
    function get_data(keyword){
        ...
        AJAX.select.table('tbl_division'); //selecting table
        ...
        AJAX.select.exec(function(result){
           ...
           if(obj.length > 0){
             $.each(obj,function(x,y){
                ...
                html += '   <td>'+y.name+'</td>';
                html += '   <td>'+status+'</td>';
                html += '   <td><a  href="<?= base_url()."content_management/"?>site_division/update/'+y.id+'" data-id ="'+y.id+'" class="app_class">Edit</a></td>'
                html += '</tr>'
             });
    ...
    $(document).on('click','.btn_add',function(e){
        location.href = ('<?= base_url()."content_management/"?>site_division/add');
    })
