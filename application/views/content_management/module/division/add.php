...
$(document).on('click', '#btn_save', function(){
        ...
        if(validate.all()){
            if(!is_exists('tbl_division', 'name', $('#name').val(), 'status') != 0){
              var modal_obj = '<?= $this->standard->confirm("confirm_save"); ?>'; 
                modal.standard(modal_obj, function(result){
                    if(result){
                        modal.loading(true);

                        AJAX.insert.table("tbl_division");
                    ...
                    var error_message2 = "<span class='validate_error_message' style='color: red;'>Division Name Exists<br></span>"
                    $('#name').css('border-color','red');
                    $(error_message2).insertAfter($('#name'));
            }
        }
});
