...
$(document).on('click', '#btn_save', function(){
        ...
        if(validate.all()){
            if(!is_exists('tbl_category', 'name', $('#name').val(), 'status') != 0){
              var modal_obj = '<?= $this->standard->confirm("confirm_save"); ?>'; 
                modal.standard(modal_obj, function(result){
                    if(result){
                        modal.loading(true);

                        AJAX.insert.table("tbl_category");
                    ...
                    var error_message2 = "<span class='validate_error_message' style='color: red;'>Category Name Exists<br></span>"
