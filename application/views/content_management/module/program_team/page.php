...
<script type="text/javascript">
    ...
    function get_data(keyword){
        ...
        AJAX.select.table('tbl_program_teams'); //selecting table
        AJAX.select.select('id,team_name,status'); //selecting result : not accepting *

        if(keyword) {
            AJAX.select.query(" (team_name like '%"+keyword+"%') and status >= 0");
        }else{
            AJAX.select.where.greater_equal("status",0); 
        }

        AJAX.select.order.asc("team_name"); 
        AJAX.select.exec(function(result){
           ...
           if(obj.length > 0){
             $.each(obj,function(x,y){
                ...
                html += '<tr>';
                html+="      <td class='text-center'><input class = 'select'  data-id = '"+y.id+"' data-name='"+y.team_name+"' type ='checkbox'></td>";
                html += '   <td>'+y.team_name+'</td>';
                html += '   <td>'+status+'</td>';
                html += '   <td><a  href="<?= base_url()."content_management/"?>site_program_team/update/'+y.id+'" data-id ="'+y.id+'" class="app_class">Edit</a></td>'
                html += '</tr>'
             });
    ...
    $(document).on('click','.btn_add',function(e){
        location.href = ('<?= base_url()."content_management/"?>site_program_team/add');
    })
