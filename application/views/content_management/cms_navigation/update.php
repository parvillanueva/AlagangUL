...
<script type="text/javascript">
	...
	function get_data() {
	    ...
	    var data = {
	    	event : "list",
	        select : "id,menu_name,menu_icon,menu_type,menu_status",
	...
	function get_roles(){
		...
		var data = {
			event : 'list',
			select : 'cms_user_roles.id as role_id,name,status,cms_menu_roles.role_id as menu_role_id,menu_id,menu_role_read,menu_role_write,menu_role_delete',
			table: 'cms_user_roles',
			query: 'menu_id = '+menu_id+' AND status = 1',
            join : [ //optional
                {
                    table : "cms_menu_roles", //table
                    query : "cms_menu_roles.role_id = cms_user_roles.id", //join query
	...
	function update_data(){
		...
		if(validate.standard('menu')){
			...
			if($('.type').val() == 2){
				if(old_title_id > 20){
					urls = 'content_management/site_'+$('#name').val().replace(' ','_');
				}else{
					urls = old_url;
				}		
			}
			    ...
			    aJax.post(url,data,function(result){
				    var obj = is_json(result);
					    if(obj == "success"){
					    	update_menu_roles();
						    	if(urls == "#"){
						    		modal.alert("Successfuly updated",function(){ 
							    		location.href = '<?=base_url('content_management/cms_menu/menu');?>';
							    	});
						    	}else{
						    		if(old_type != $('.type').val()){
						    			url2 = "<?= base_url("content_management/preference/navigation/update_menu");?>";	
										data2 = {
											menu : $('#name').val()
										}
	...
	$(document).on('change','.type',function(){
		if($(this).val() == 1)
		{
		    ...
		    if(hasUnder == 2)
		    {
		        modal.alert("<?= $this->standard->dialog("menu_hasUnder"); ?>", function(){
